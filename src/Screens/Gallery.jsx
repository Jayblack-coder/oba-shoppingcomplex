...
//               rgba(15, 15, 15, 0.72)
//             ),
//            url("${gallery[0]?.url || FALLBACK_IMAGE}")
//           `,
...
import { useCallback, useEffect, useMemo, useState } from "react"; 
import { Box, Button, Chip, CircularProgress, Container, Dialog, DialogContent, IconButton, Stack, Typography, } from "@mui/material"; 
import CloseIcon from "@mui/icons-material/Close"; 
import ArrowBackIosNewIcon from "@mui/icons-material/ArrowBackIosNew"; 
import ArrowForwardIosIcon from "@mui/icons-material/ArrowForwardIos"; 
import ZoomInIcon from "@mui/icons-material/ZoomIn"; 
...
export default function Gallery() { 
  ...
  const filteredGallery = useMemo(() => { 
    ...
    [gallery, selectedType]);

    const heroImage = gallery.find((item) => 
      item.type === "image")?.url || FALLBACK_IMAGE;
    const selectedMedia = selectedMediaIndex 
    !== null ? filteredGallery[selectedMediaIndex] : null; 
    const openLightbox = (index) => { setSelectedMediaIndex(index); }; 
    const closeLightbox = useCallback(() => { setSelectedMediaIndex(null); }, []);
    const showPreviousMedia = useCallback(() => { if (!filteredGallery.length) 
      return; setSelectedMediaIndex((currentIndex) => { if (currentIndex === null) 
        return null; 
        return currentIndex === 0 ? filteredGallery.length - 1 : currentIndex - 1; }); }, [filteredGallery.length]); 
        const showNextMedia = useCallback(() => { if (!filteredGallery.length) 
          return; setSelectedMediaIndex((currentIndex) => { if (currentIndex === null) 
            return null; 
      return currentIndex === filteredGallery.length - 1 ? 0 : currentIndex + 1; }); }, [filteredGallery.length]);
      useEffect(() => { if (selectedMediaIndex === null) return; 
        const handleKeyDown = (event) => { if (event.key === "ArrowLeft") { showPreviousMedia(); } 
      ...
      return () => { 
        ...
        [ selectedMediaIndex, showPreviousMedia, showNextMedia, closeLightbox, ]);
        ...
        if (loading) { 
          ...
          <Stack spacing={2} alignItems="center"> 
            ...
            <Typography color="text.secondary"> Loading gallery... 
              ...
              return ( 
              <Box 
              sx={{ bgcolor: "#faf9f6", minHeight: "100vh", }} > 
              {/* ===================================== 
              HERO SECTION 
              ===================================== */} 
              <Box 
        ...
        <Container id="gallery-content" maxWidth="xl" sx={{ py: { xs: 7, md: 10, }, }} > {/* Heading */} 
          <Box sx={{ textAlign: "center", mb: 5, }} > 
            <Typography variant="h3" fontWeight={800} sx={{ fontSize: { xs: "2rem", md: "3rem", }, }} > Explore Our Plaza 
              ...
              {["All", "Image", "Video"].map( (type) => { const active = selectedType === type; 
                return ( 
                <Button 
                key={type} 
                ...
                sx={{ 
                  ...
                  { bgcolor: "#D4AF37", color: "#111", "&:hover": { bgcolor: "#b99425", }, } : 
                  { borderColor: "rgba(0,0,0,0.18)", color: "#333", "&:hover": 
                  { borderColor: "#D4AF37", color: "#b99425", }, }), }} > 
                  ...
                  {error && ( 
                    <Box
                     ...
                     <ImageNotSupportedIcon 
                     sx={{ fontSize: 60, color: "#D4AF37", mb: 2, }} />
                      <Typography variant="h5" fontWeight={700} >
                         Gallery unavailable 
                         </Typography> 
                         <Typography color="text.secondary" mt={1} > {error} 
                          </Typography> 
                          <Button 
                          variant="contained" 
                          onClick={fetchGallery} 
                          sx={{ 
                            mt: 3, bgcolor: "#D4AF37", color: "#111", "&:hover": 
                            { bgcolor: "#b99425", }, }} > 
                            Try Again 
                            </Button>
                             </Box> )} 
                             ...
                             {!error && filteredGallery.length === 0 && ( 
                              <Box sx={{ textAlign: "center", py: 10, }} >
                                 <CollectionsIcon 
                                 sx={{ fontSize: 70, color: "text.disabled", }} />
                                  <Typography variant="h5" fontWeight={700} mt={2} > 
                                    No media available 
                                    </Typography>
                                     <Typography color="text.secondary" mt={1} > 
                                      Gallery images and videos will appear here once they are uploaded. 
                                      </Typography> 
                                      </Box> )} 
                                      {/* ===================================== 
                                      MASONRY GALLERY 
                                      ...
                                      {!error && filteredGallery.length > 0 && ( 
                                        ...
                                        sx={{ 
                                          ...
                                          columnGap: 3, }} > 
                                        {filteredGallery.map( (item, index) => ( 
                                          <Box key={ item._id || `${item.url}-${index}` } 
                                          onClick={() => openLightbox(index) } 
                                          sx={{ 
                                            position: "relative", 
                                            mb: 3, 
                                            ...
                                            sx={{ 
                                              position: "absolute", 
                                              inset: 0, display: "flex", flexDirection: "column", justifyContent: "flex-end", p: 3, opacity: { xs: 1, md: 0, }, 
                                              transition: "opacity .35s ease", 
                                                      ...
                                                      <Typography sx={{ color: "#D4AF37", textTransform: "uppercase", letterSpacing: 1.5, fontSize: ".8rem", fontWeight: 700, }} > {selectedMedia.type === "video" ? "Video" : "Gallery Image"} </Typography> <Typography variant="h5" sx={{ color: "#fff", fontWeight: 700, mt: 0.5, }} > {selectedMedia.title || "Sweet Asouzu Plaza"} 
                                                        ...
                                                        <Typography sx={{ color: "rgba(255,255,255,.45)", mt: 1, fontSize: ".85rem", }} > {selectedMediaIndex + 1}{" "} of{" "} {filteredGallery.length} </Typography> 
                                                          ...
                                                          sx={{ 
                                                            ...
                                                            zIndex: 20, bgcolor: "rgba(255,255,255,.12)", 
